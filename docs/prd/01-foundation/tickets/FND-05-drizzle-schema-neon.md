...
- v0.2 (2026-07-23, FIT-01 Builder writeback — **Feedback obligation #1 fired**: a jsonb column's DB shape diverging from its Zod counterpart): `jobs.ledger` and `jobs.fit` are now **DB-nullable**; `jobs.jd` is unchanged (`NOT NULL`). The Zod `Job` in `lib/schemas/persisted.ts` is unchanged and stays non-nullable — the divergence is deliberate and is documented in `db/schema.ts`'s `jobs` comment block and in `lib/db/queries/jobs.ts`'s `PersistedJob`.
  - **Reason**: Fit is one user-facing operation delivered as two server calls — FIT-01's `POST /api/jobs` creates the row with `jd` only, FIT-02's `POST /api/jobs/[id]/fit` fills `ledger` + `fit` together. FND-04's Changelog v0.2 and `docs/plans/FIT-01.md` §0.1 carry the full argument and the rejected alternatives. Horace's sign-off is the FIT-01 merge.
  - **Changed files**: `db/schema.ts` (two `.notNull()` removals + a rewritten comment block); `db/migrations/0003_perpetual_sunset_bain.sql` + `meta/0003_snapshot.json` + `meta/_journal.json`, all **generated** by `pnpm db:generate`, never hand-edited — the emitted SQL is exactly `ALTER TABLE "jobs" ALTER COLUMN "ledger" DROP NOT NULL;` and the same for `"fit"`.
  - **Two upstream tests updated in the same commit**: `db/schema.test.ts`'s NOT-NULL assertion now pins `jd` NOT NULL / `ledger` + `fit` nullable. `db/migrate.test.ts`'s Tier-2 assertion was rewritten because it greps the **concatenation of every** migration — `/"ledger" jsonb NOT NULL/` still matches migration `0000` and the test would have stayed **green while asserting a fact untrue of the live schema**; it now pins where the chain *ends* (`ALTER TABLE ... DROP NOT NULL` present, and no such statement for `jd`). Tier 3's round-trip, which previously proved a `fit`-less insert is rejected, now proves the opposite half: a `jd`-only insert succeeds and reads back nulls, while an insert missing `jd` is still rejected.
  - **Known stale consumer, deliberately NOT fixed here** (`07-platform-launch`-owned, already merged): `lib/db/queries/admin.ts:333–340` computes `fitToTailor` with a denominator of *all* jobs, justified by a comment reading "jobs.fit is NOT NULL in db/schema.ts:174". That comment is now false and the metric will count `jd`-only jobs that never produced a Fit report. Raised as a follow-up for Horace (`docs/plans/FIT-01.md` §5 Q2) — a one-line `isNotNull(jobs.fit)` filter in a PLT ticket, not a cross-module edit from FIT-01.
