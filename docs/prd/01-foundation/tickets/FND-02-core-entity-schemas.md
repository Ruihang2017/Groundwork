...
- v0.2 (2026-07-18, FND-02 Builder writeback): widened `vitest.config.ts`'s `test.include` from `['tests/**/*.test.ts']` to `['tests/**/*.test.ts', 'lib/**/*.test.ts']`. The inherited FND-01 config only matched `tests/**`, so this ticket's own specified test file location (`lib/schemas/entities.test.ts`, named three times in File-scope/Deliverables/Test-plan) was unreachable — `pnpm test` would have reported green while running zero of this ticket's assertions (a false-green against acceptance item 4). The widening is the minimal fix and additionally unblocks FND-03/04/06/07/10's colocated `lib/**` test files without each re-touching this shared config. Also recorded: `Profile.contact.email` uses Zod v4's non-deprecated top-level `z.email()` rather than the ticket's literal `z.string().email()` snippet (identical validation; the literal form is deprecated in v4 and slated for removal); resolved `zod` version pinned at `^4.4.3` (latest stable at implementation time).
- v0.1 (2026-07-17): initial draft, generated by `/breakdown-prd`.
