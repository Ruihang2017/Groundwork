...
- v0.2 (2026-07-23, FIT-01 Builder writeback — **Feedback obligation #2 fired**): `04-fit`/FIT-01's implementation found the `Job.jd`/`ledger`/`fit` non-nullability decision unworkable **at the database level**, exactly the case obligation #2 anticipated. Resolution taken (recorded here rather than fixed silently, per that obligation):
  - **`lib/schemas/persisted.ts` is UNCHANGED.** The Zod `Job` still requires all three fields. It remains the *complete-Job API contract*: a complete `Job` is only ever returned over the API once FIT-02 has finished, and every acceptance item of this ticket still holds.
  - **The DB-level mirror was relaxed** for `ledger`/`fit` only (`db/schema.ts`, migration `0003_perpetual_sunset_bain.sql` — see FND-05's Changelog v0.2). `jd` stays `NOT NULL`.
  - **Why**: "Fit" is one user-facing operation delivered as TWO server calls. FIT-01's `POST /api/jobs` creates the row from READ's output alone; FIT-02's route is `POST /api/jobs/[id]/fit`, and a job **id in the path** means the row must already exist; FIT-03 Deliverable 7 explicitly renders the in-between state ("Generating your Fit Report…"). The atomicity assumed here and in `04-fit/README.md`'s original 决策 row 2 contradicted all three of `04-fit`'s own tickets.
  - **The persistence contract is now weaker than the API contract**, deliberately: `PersistedJob` (= `Job` with `ledger`/`fit` nullable) lives in `lib/db/queries/jobs.ts`, module-local per `breakdown-plan.md` §3. A row may be transiently incomplete; a `Job` over the wire may not.
  - Evidence trail and the rejected alternatives (R-B re-cut the three tickets; R-C persist placeholder zeroed values — rejected outright as indistinguishable from a real Fit report): `docs/plans/FIT-01.md` §0.1. Decision authority is Horace, and **merging FIT-01 is the sign-off**.
