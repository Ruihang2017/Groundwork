...
- v0.6（2026-07-23，PLT-04 Builder writeback）：PLT-04（邀请码注册门控）Deliverables 1–5 实现完成，本模块 4 张票据全部构建完毕。全套测试 **682 通过（61 文件）**，其中新增 54 项 / 4 个新文件；`pnpm lint` clean；**完全无 env var** 的 `next build` exit 0（middleware bundle 正常产出，清掉 plan §5 Q1）。新增 `db/schema.ts` 的 `invite_codes` 表（migration `0004_clear_gressill.sql`）、`lib/db/queries/invite-codes.ts`、`scripts/generate-invite-codes.mjs`，并向 `auth.ts`、`app/(auth)/signin/page.tsx` 追加（append-only）。要点（完整 deviations 见 `tickets/PLT-04-invite-codes.md` 的 Changelog v0.1）：
  - **票据 Deliverable 4/5 的两处前提在 Auth.js v5 下不成立，已按事实实现并记录**：`signIn` callback 既不接收 `isNewUser` 信号（该字段只存在于 `events.signIn` 消息，触发时登录已提交、拦不住），也拿不到任何 request 对象或 `signIn()` 的额外 options（它们只进 POST body）。因此「新用户判定」改用与 `@auth/core` 自身完全相同的 `users.email` **精确匹配**查询（必须大小写敏感——大小写不敏感的「已存在」会在上游仍会 `createUser` 的情形下静默放行，即门控被绕过），邀请码则通过一枚短生命周期（30 分钟）、`samesite=lax` 的 cookie 传输，由 callback 内**动态** import 的 `next/headers` 读取。`lax` 不可改为 `strict`：Google 回跳是顶层跨站 GET，`strict` 会丢 cookie，所有 Google 新注册静默失败。
  - **两阶段 claim/attach**：门控运行在 `users` 行**创建之前**，且 `@auth/drizzle-adapter` 会丢弃传入的 `id`，所以此刻可见的 `user.id` 永远不会出现在 `users` 表里（写入即触发 `used_by` 外键错误，已有测试断言其 reject）。故 `redeemInviteCode` 的签名放宽为 `(code, userId: string | null)`：生产路径传 `null` 原子占用邀请码，`events.createUser` 事后 best-effort 回填 `used_by`（该 hook 被 `@auth/core` await 在 try 块内，因此**绝不能抛异常**，否则会让一个账号已创建成功的登录失败）。
  - **P0 并发保证**：`redeemInviteCode` 是单条带守卫的 `UPDATE … WHERE code = $1 AND used_at IS NULL RETURNING`，禁止改写为「先读后写」（模块头以此措辞明令禁止）。查询层与门控层各有一个并发测试，均断言两个并发请求恰有一个成功。**诚实边界**：PGlite 单连接串行执行，这些测试证明的是守卫 UPDATE 的行数语义，而非真正的并行行锁竞争——即票据验收项 3 明文允许的替代方案。**Feedback obligation #1 未解除**：P5 sign-off 前仍需在真实 Neon 实例上做并发验证。
  - **`used_at IS NULL`（而非 `used_by IS NULL`）是唯一权威的「未使用」判据，`onDelete: set null` 是必需项而非风格选择**：PLT-01 的账号硬删以单事务 `DELETE FROM users` 收尾，若沿用 drizzle 默认引用动作，任何用过邀请码的用户都将**无法删除账号**（直接破坏 PRD §5.6）；而 `SET NULL` 会在删号时清空 `used_by`，因此以 `used_by` 为判据会让已用邀请码在其使用者删号瞬间静默回到流通。已有测试执行真实 `DELETE FROM users` 并断言三项后果。`app/api/account/delete/route.ts` **无需改动**。
  - **邮件登录在 phase 1 门控、phase 2 有意放行**：Resend provider 会调用 callback 两次；phase 2（点开魔法链接）必须持有 phase 1 成功后才生成的一次性、`AUTH_SECRET` 哈希、24h 有效的 verification token，故在此重复门控不增加任何安全性，反而会打断跨设备开链接的正常场景并二次校验一枚已消费的码。**代价（plan §5 Q1'，待 Horace 判断）**：邀请码在邮件**发出**时即被消费，写错地址或收件人从未点开都会烧掉一枚码。这是刻意取舍——那是唯一能确保码存在的时刻，且烧码可恢复（再发一枚），双重兑换不可恢复。
  - **File-scope deviations（已标记给 Reviewer）**：新增 3 个票据字面未列的测试文件（沿用 FND-08 `db/schema-auth.test.ts`、PLT-02 `tests/backup.test.ts` 的先例），且**零编辑任何既有测试文件**（`auth.test.ts` / `auth.config.test.ts` / `page.test.tsx` / `db/*.test.ts` 全部逐字节不变，作为回归守卫）；另向 `vitest.config.ts` 追加 `hookTimeout: 30_000`——本票据新增的两个 PGlite 套件把三个沿用 Vitest 默认 10s 的**既有**文件挤爆（ISS-29 同类负载 flake，已复现并修复），抬高全局下限可在**不改动他票据所属文件**的前提下一次修好；该改动只放宽期限，不改变任何断言，也不可能把红测试变绿。
  - **生成器安全性**：`node:crypto.randomInt` + 31 字符去混淆字母表 × 长度 10 ≈ 2^49.5；`Math.random` 由一条 grep 脚本可执行源码的测试明令禁止。本仓库没有登录限流、本票据也没有加——v1 规模下（在售码数十枚，每次猜测成本是一整轮 OAuth/魔法链接往返）暴力破解不构成可信路径；**若日后缩短字母表或长度，此论证必须重做**。脚本在 `DATABASE_URL` 缺失时**显式失败**（有意不采用 PLT-02 的静默 no-op——那是为无人值守 CI 设计的，人工运维工具静默「成功」是陷阱），并且**绝不打印任何非本次新插入的码**。
  - **本机遗留失败**：`tests/backup.test.ts` 在此 Windows 开发机上因 `EBUSY … rmdir …iss31-bashprobe-*`（PLT-02 的 ISS-31 探针临时目录）失败，且在 `main` 上以完全相同方式失败——属既有的环境相关问题，与本票据无关，未作处理（该文件属 PLT-02 file-scope）。
