...
| `Job.jd`/`ledger`/`fit` 三字段非空是数据完整性保证，CROSS+SCORE 必须在同一次请求内原子完成，不允许"有 ledger 无 fit"的中间态被持久化 | §5.6 `Job` schema 三字段均非 nullable；FND-04 已据此建 schema，本模块的 FIT-02 是唯一写入这两个字段的路由 |
