...
- **2026-07-24 (Builder writeback).** Implemented per `docs/plans/PRP-04.md` on `ticket/PRP-04` (branched from `main` @ `b32a7b6`). AI-generated draft — must be reviewed by the Reviewer stage before merge.

  **What landed (all under `app/(app)/jobs/[id]/prep/`, this module's exclusive directory — zero cross-module imports, confirmed by `git grep`):**
  - Extended `page.tsx`'s unlocked (`interviewing`) branch: reads `getBrief` + `getLibrary` (both session-scoped, both import-safe with `DATABASE_URL` unset), then either renders `<BriefView>` server-side (a `Brief` exists — the reload path, zero fetch, acceptance item 5) or hands off to `<BriefGenerator>` (no `Brief` yet). The locked branch + all guard scaffolding (`requireUserId`/`getJob`/`notFound()`/`dynamic`/`await params`) are preserved byte-for-byte in behaviour (PRP-03 Feedback obligation #2).
  - Deliverable components: `research-fail-banner.tsx` (Deliverable 1), `intel-card.tsx` (Deliverable 2), `question-list.tsx` (Deliverable 3, grouped by `projectId`/"angle"), `ask-them-list.tsx` (Deliverable 4), `positioning-summary.tsx` (Deliverable 5), `dropped-count-header.tsx` (Deliverable 6 — this module's own copy, no `partial` flag, TLR-02 shape).

  **Non-enumerated but structurally-required files (flagged here exactly as PRP-01/PRP-02 flagged their additive gates/envelopes; all within `prep/**`, no cross-module reach):**
  - `_components/brief-view.tsx` — the single composition point rendered by BOTH the server reload path and the client generator (the `fit-report-view.tsx` analog); without it the two paths could silently drift in order/content. NOT a client component.
  - `_components/brief-generator.tsx` — `'use client'`; the RESEARCH→REHEARSE orchestrator (the `fit-auto-runner.tsx` analog). A server component cannot issue a client-triggered POST sequence with a progress UI.
  - `_lib/project-names.ts` — this module's own `projectNameMap` (per-module duplication, `breakdown-plan.md` §3; mirrors `resume/_lib/project-names.ts`).
  - `_fixtures/brief-fixtures.ts` — test-only, hand-written, type-only-import fixtures (`job-fixtures.ts` conventions).

  **Decisions implemented (full detail in the plan's decision table; each also appears as a code comment at its site):** D1 auto-fire on mount (not click); D2 orchestration = RESEARCH-first with a sharp degrade-vs-hard-fail split (200/`failed:true` OR malformed-200 → proceed to REHEARSE with the captured `intel`; NON-200 → hard stop, REHEARSE never called — closes the cost hole where a free REHEARSE would mint a paid brief after a RESEARCH `429`); D3 render-from-response (no `router.refresh`); **D4 the load-bearing relaxed read** — the REHEARSE-200 response schema uses `questions: .max(5)` (not `.length(5)`), so a referential-integrity-dropped-question brief is a SUCCESS, not a false error (lands ADR-A's read-path consequence pre-registered by PRP-02); D5 rehearse-phase retry re-runs REHEARSE only (reusing the captured `intel`, avoids double-charging `prep`); D6 single-flight + no auto-retry; D7 dropped-count-header copies TLR-02's no-`partial` shape; D8 `brief-view.tsx` is a server component; D9 banner/card self-guard and both render unconditionally so the banner appears ALONGSIDE the brief (acceptance item 3); D10 group questions by `projectId` in first-appearance order with a raw-id fallback; D11 minimal inline ledger recap (strengths count + each gap's probe/play, injected-gap-safe); D12 module-local `project-names`; D13 two-phase `role="status"` progress line (streaming is not implemented — the honest substitute, same gap `fit-auto-runner` records).

  **Two known limitations (both inherent, NOT worked around — see Feedback obligations #1/#2 and the plan's Q1/Q5):**
  1. **Dropped-count header is render-once.** `droppedCount` is not part of the persisted `Brief` schema (FND-04), so the dropped-count header shows only right after generation (from PRP-02's response `dropped` envelope) and NOT on subsequent visits to an already-generated brief. The durable fix (a `droppedCount` column on `Brief`) is a foundation-schema change owned by Horace + `01-foundation` — NOT faked with disappearing client state.
  2. **Auto-fire re-charges `prep` across visits until success.** No `Brief` is persisted until REHEARSE succeeds, so a persistently-failing generation re-fires (and RESEARCH re-charges a `prep` unit) on each fresh unlocked visit, bounded by 3/day → 429. This mirrors `fit-auto-runner.tsx`'s documented limitation; escalate to Horace via a README open question if P4 dogfood shows real pain, rather than silently switching to a click-trigger (which would break acceptance item 1's "on render").

  **Scope boundary (unchanged, recorded):** no user-facing "regenerate" button for an already-generated `Brief` (Deliverable 7 note + Feedback obligation #1). REHEARSE fires once per job; a later visit reloads the persisted brief with zero fetch.

  **Results (real output, not "should pass"):**
  - `corepack pnpm test` — **104 files / 1239 tests, all green** (baseline was 95/1195; this ticket adds 9 test files / 44 tests). No pre-existing test changed except `page.test.tsx` (unlocked-branch assertions rewritten from PRP-03's transient placeholder per Feedback obligation #2; locked-branch + auth/scoping/build-guard regressions unchanged and still green).
  - `corepack pnpm lint` — **clean**.
  - `corepack pnpm build` with `DATABASE_URL` unset — **exit 0**, `/jobs/[id]/prep` present in the route table (4.44 kB First Load JS 128 kB); confirms the build-guard (lazy `dbIndex`) and Next-15 async-`params` paths.
  - Cross-module-import check (`git grep` per plan §3) — **zero hits** (the single grep match is a comment in `lock-screen.tsx`, not an import).

  **Honesty note:** every test here stubs `fetch` and hand-writes every fixture, so green proves the WIRING (call order, degrade routing, single-flight, the relaxed read, render composition) and nothing about the real quality of an interview brief. The live end-to-end brief (a real RESEARCH+REHEARSE against a real job) is Horace's P4 dogfood `[human]` acceptance (this ticket's acceptance item 7).
